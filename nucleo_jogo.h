...
bool verificarPalavraValida(string palavra, string nomeArquivo, string& palavraSorteada,TPilha palavrasAcertadas) {
    ...
    // verifica se a palavra digitada já é a palavra sorteada
    // se for a palavra sorteada: pontuação máxima, reinicia tempo e sorteia nova palavra
    ...
    // Percorre a pilha de palavras já acertadas
    // Se a palavra já foi digitada antes, não pontua novamente
    ...
    if (!arquivo.is_open())
        return false;
    // Se está no dicionário: pontuação e tempo extra.
    // Se não está no dicionário: não pontua, retorna falso
    string linha;
    while (getline(arquivo, linha)) {
        if (linha == palavra) {
            arquivo.close();
